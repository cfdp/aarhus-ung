...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        LoadDamp('<%=this.ClientID%>', '<%=HiddenFieldIds.ClientID%>', '<%=SelectMultipleNodesValue.ToString().ToLower()%>', '<%=ThumbnailHeightValue.ToString()%>');
    });

    function GetSelect_<%=this.ClientID%>(nodeIds) {
        if(nodeIds.outVal){
            GetSelectDamp('<%=this.ClientID%>', '<%=HiddenFieldIds.ClientID%>', '<%=SelectMultipleNodesValue.ToString().ToLower()%>', '<%=IOHelper.ResolveUrl(SystemDirectories.Umbraco)%>', nodeIds, '<%=ThumbnailHeightValue.ToString()%>', '<%=ThumbnailWidthValue.ToString()%>', '<%=CropPropertyAliasValue%>', '<%=CropNameValue%>', '<%=DigibizConstants.IMAGETYPES%>', '<%=HideEditValue.ToString().ToLower()%>', '<%=HideOpenValue.ToString().ToLower()%>', '<%=HidePixlrValue.ToString().ToLower()%>', 'UpdateSelect_<%=this.ClientID%>');
        }
    }

    function UpdateSelect_<%=this.ClientID%>(nodeId) {
        if(nodeId.outVal){
            UpdateSelectDamp('<%=this.ClientID%>', '<%=HiddenFieldIds.ClientID%>', '<%=IOHelper.ResolveUrl(SystemDirectories.Umbraco)%>', nodeId, '<%=ThumbnailHeightValue.ToString()%>', '<%=ThumbnailWidthValue.ToString()%>', '<%=CropPropertyAliasValue%>', '<%=CropNameValue%>', '<%=DigibizConstants.IMAGETYPES%>', '<%=HideEditValue.ToString().ToLower()%>', '<%=HideOpenValue.ToString().ToLower()%>', '<%=HidePixlrValue.ToString().ToLower()%>', 'UpdateSelect_<%=this.ClientID%>');
        }
    }
